...
The goal of this assignment is to get you warmed up with git, GitHub, markdown, and R. The first few seminars will cover all you need to know for completing this practice assignment.AnnaHLA
...
#The total number of children on Titanic was found by summing the number in the Freq column if they are categorized as Child in the Age column. The same was done to find the number of adults, by swithing from Child to Adult in the Age column.

children_total = sum(titanic_df$Freq[titanic_df$Age == "Child"])
adults_total = sum(titanic_df$Freq[titanic_df$Age == "Adult"])

#The total number of male and female adults was found by summing up the number of adults of female and male sex, respectively. It was found that there were more male than female adults on Titanic 'r male_total' men compared to 'r women_total' women.
...
eplace this text with an explanation of your code output to answer the questions. 
